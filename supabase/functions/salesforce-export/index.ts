...
━━━━━━━━━━━━━━━━━━━━━━━━━━`;
    ...
    if (!updateResponse.ok) {
      ...
      // If record not found (404), clear the Salesforce ID to prevent future errors
      if (updateResponse.status === 404) {
        console.log(`🔄 Record not found, clearing Salesforce ID for: ${leadData.email}`);
        
        // Update the contact record to clear the invalid Salesforce ID
        const { error: updateError } = await supabase
          .from('contacts')
          .update({ 
            salesforce_id: null,
        ...
        if (updateError) {
          console.error('Failed to clear Salesforce ID:', updateError);
        }
        
        return new Response(JSON.stringify({
          success: false,
          message: 'Salesforce record not found - contact updated to remove invalid reference',
          action: 'cleared_invalid_id',
          recordId: leadData.salesforce_id
        }), {
          status: 200, // Changed to 200 since we handled the issue gracefully
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
