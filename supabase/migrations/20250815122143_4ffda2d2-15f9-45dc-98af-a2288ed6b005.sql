-- Continue creating the enterprise CRM schema (skipping user_role enum)
...
-- Insert default sales stages if they don't exist
INSERT INTO public.sales_stages (name, probability, stage_order, is_closed_won, is_closed_lost) 
SELECT * FROM (VALUES
...
) AS v(name, probability, stage_order, is_closed_won, is_closed_lost)
WHERE NOT EXISTS (SELECT 1 FROM public.sales_stages);
