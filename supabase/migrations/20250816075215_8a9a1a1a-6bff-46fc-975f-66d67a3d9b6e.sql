-- Comprehensive security fixes for all identified vulnerabilities (corrected)
...
VALUES 
  ...
  -- Token tables (critical) - using 'restricted' for highest security
  ('salesforce_tokens', 'access_token', 'restricted', true, true),
  ('salesforce_tokens', 'refresh_token', 'restricted', true, true),
  ('hubspot_tokens', 'access_token', 'restricted', true, true),
  ('hubspot_tokens', 'refresh_token', 'restricted', true, true),
  
  -- OAuth sensitive data
  ('oauth_states', 'code_verifier', 'restricted', true, true),
  ('oauth_states', 'state', 'restricted', true, true)
...
DROP TRIGGER IF EXISTS audit_contacts_access ON public.contacts;
...
DROP TRIGGER IF EXISTS audit_salesforce_tokens_access ON public.salesforce_tokens;
CREATE TRIGGER audit_salesforce_tokens_access
  AFTER INSERT OR UPDATE OR DELETE ON public.salesforce_tokens
  FOR EACH ROW EXECUTE FUNCTION public.audit_sensitive_access();

DROP TRIGGER IF EXISTS audit_hubspot_tokens_access ON public.hubspot_tokens;
