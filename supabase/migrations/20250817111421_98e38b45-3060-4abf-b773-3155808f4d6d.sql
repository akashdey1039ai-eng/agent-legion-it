...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_owner_created 
ON public.contacts(owner_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_salesforce_sync 
ON public.contacts(salesforce_id, last_sync_at) 
WHERE salesforce_id IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contacts_hubspot_sync 
ON public.contacts(hubspot_id, last_sync_at) 
WHERE hubspot_id IS NOT NULL;

-- Add composite indexes for AI test tracking
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ai_test_progress_compound 
ON public.ai_test_progress(test_run_id, platform, agent_type, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ai_test_runs_user_status 
